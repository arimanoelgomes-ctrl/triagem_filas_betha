# Logs de Execução — Triagem Saúde

Esta pasta armazena o histórico diário de execução da triagem da **vertical Saúde**. Cada execução gera um arquivo Markdown no formato `YYYY-MM-DD.md`.

## Finalidade

- **Auditoria** — rastrear o que foi analisado, ignorado ou comentado em cada dia.
- **Qualidade** — permitir revisão amostral pela referência da vertical (Maitê Passos).
...
```markdown
# Triagem Automática Saúde — YYYY-MM-DD
...
- **Sistema:** PEC / e-SUS APS / outro
- **Município/Cliente:** Itajaí (ou nome do projeto)
...
- Nunca incluir nos logs dados sensíveis do paciente/cidadão (CPF/CNS completos, dados clínicos, endereços etc.). Quando necessário citar, anonimize.
